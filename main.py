...
ACTIVE_HOURS = range(8, 19)  # 08:00–18:59 Europe/Rome: window for fetch/parse
EOD_HOUR = 17                # send recap on first run at/after this Rome hour
...
def main():
    ...
    # EOD recap is decoupled from ACTIVE_HOURS: a cron run delayed past 18:59 Rome
    # (GitHub Actions cron routinely slips) must still be able to fire the recap.
    in_active_hours = now.hour in ACTIVE_HOURS
    in_eod_window = now.hour >= EOD_HOUR

    if not in_active_hours and not in_eod_window:
        print(f"[{now.isoformat()}] outside active and EOD windows; exiting.")
        return 0
    ...
    if in_active_hours:
        ...
        print(f"[{now.isoformat()}] EOD-only run (outside active hours).")
